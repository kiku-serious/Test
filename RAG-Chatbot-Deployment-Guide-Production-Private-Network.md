...
* **フロントエンド (Streamlit on Amazon ECS Fargate with ALB):**
    ...
    * **ECS Fargate**: Streamlitアプリの実行環境。複数のユーザーに対応するため、必要に応じてタスクが自動で増減（オートスケーリング）します。**完全にプライベートサブネットに配置され、パブリックIPは持ちません**。
...
* **既存ネットワークインフラストラクチャ**:
    ...
    * **サブネット**: パブリックサブネット（ALB配置用）とプライベートサブネット（ECSタスク、Lambda、VPCエンドポイント配置用）を適切に使い分けます。
...
このシステムは既存VPC内に構築されるため、まず以下の情報を正確に把握しておいてください。
...
6.  **既存のVPCエンドポイントと関連セキュリティグループ**:
    ...
    * これらのVPCエンドポイントのIDと、関連付けられているセキュリティグループID（例: `sg-api-gateway-endpoint`, `sg-bedrock-vpce`, `sg-dynamodb-vpce`, `sg-cloudwatch-logs-vpce`）を控えます。もし存在しなければ、構築時に別途作成が必要です。
...
2.  **ECS タスク実行ロール (`ecsTaskExecutionRole`)**:
    ...
    * ロール名に「**`ecsTaskExecutionRole`**」と入力し、「**ロールを作成**」。ARNを控えておきましょう。
3.  **ECS タスクロール (`ecsTaskRole`)**:
    ...
    * ロール名に「**`ecsTaskRole`**」と入力し、「**ロールを作成**」。ARNを控えておきましょう。

---

### 1.2. セキュリティグループの作成 (Lambda & API Gateway & ECS連携用)

**作業の意味**: 各コンポーネント間のトラフィックを制御するための仮想ファイアウォールルールを定義します。

1.  **Lambda 関数のセキュリティグループ (`lambda-rag-processor-sg`)**:
...
2.  **API Gateway プライベートエンドポイントのセキュリティグループ (`sg-api-gateway-endpoint`) の確認/作成**:
    ...
    * もし存在しない場合は、EC2サービスで「セキュリティグループを作成」し、以下のルールを設定します。
        * 「セキュリティグループ名」に「**`sg-api-gateway-endpoint`**」と入力し、「VPC」で**既存のVPC ID**を選択。
        * **インバウンドルール**:
            * タイプ: 「**HTTPS**」、ポート範囲: `443`
            * 送信元: 後で作成する**ALB用セキュリティグループID**（`chatbot-alb-sg`）を許可。
        * **アウトバウンドルール**: デフォルトの「すべてのトラフィックを許可」でOKです。
    * 作成されたセキュリティグループIDを控えておきましょう。

3.  **ALB 用セキュリティグループ (`chatbot-alb-sg`) の作成**:
    ...
    * **アウトバウンドルール**:
        * タイプ: 「**カスタム TCP**」、ポート範囲: `8080` (StreamlitをALB経由でアクセスさせるポート)
        * 送信先: 後で作成する**ECSタスク用セキュリティグループID**（`streamlit-fargate-task-sg`）を許可。
    * 「**セキュリティグループを作成**」。作成されたセキュリティグループIDを控えておきましょう。

4.  **ECS タスク用セキュリティグループ (`streamlit-fargate-task-sg`) の作成**:
    ...
    * **インバウンドルール**:
        ...
        * ソース: 「**カスタム**」を選択し、**ALB用セキュリティグループID**（`chatbot-alb-sg`）を許可。
    * **アウトバウンドルール**:
        * タイプ: 「**HTTPS**」、ポート範囲: `443`
        * 送信先: 「**カスタム**」を選択し、**API GatewayプライベートエンドポイントのセキュリティグループID**（`sg-api-gateway-endpoint`）を許可。
        * タイプ: 「**HTTPS**」、ポート範囲: `443`
        * 送信先: **CloudWatch Logs VPCエンドポイントのセキュリティグループID**（`sg-cloudwatch-logs-vpce`）を許可。（Lambdaログ出力と同様に必要）
    * 「**セキュリティグループを作成**」。作成されたセキュリティグループIDを控えておきましょう。

---

### 1.3. API Gateway (プライベートエンドポイント) の設定
...
1.  **API Gateway 用 VPC エンドポイントの確認/作成**:
    ...
    * 「サービスカテゴリ」: 「**AWS サービス**」
    * 「サービス名」: 検索ボックスに「`execute-api`」と入力し、`com.amazonaws.<region>.execute-api` を選択。
    * 「VPC」: **既存のVPC ID**を選択。
    * 「サブネット」: **プライベートサブネットのIDを複数選択**。
    * 「セキュリティグループ」: **`sg-api-gateway-endpoint`** を選択。
    * 「ポリシー」: デフォルトの「フルアクセス」のまま。「**エンドポイントを作成**」。
...
4.  **既存のルートリソース (`/`) の確認**:
    * 左メニューで作成したAPI名（`ChatbotApiProd`）を選択。
...
5.  **POST メソッドの作成 (ルートリソース `/` に対して)**:
    ...
    * 「Lambda 関数」に「`ChatbotBackendFunctionProd`」（後で作成）と入力し、候補が表示されるので選択。
    * 「**保存**」。パーミッション追加を求められたら「**OK**」。

6.  **CORS 設定の確認と修正 (ルートリソース `/` に対して)**:
    * **現在、リソースツリーでルートリソース「`/`」が選択されていることを確認。**
...
1.  **Lambda 関数の作成**:
    ...
    * 関数が作成されたら、コードエディタに以下のPythonコードを貼り付けます。
    ...
    # 例: https://vpce-xxxxxxxxxxxxxxxxx-<api-id>.execute-api.<region>[.amazonaws.com/prod/](https://.amazonaws.com/prod/)
    API_GATEWAY_URL = os.environ.get("API_GATEWAY_URL_FOR_LAMBDA", "[https://your-private-api-gateway-endpoint.execute-api.your-region.amazonaws.com/prod/](https://your-private-api-gateway-endpoint.execute-api.your-region.amazonaws.com/prod/)")
    # 注意: 上記のURLはAPI Gatewayの「ステージ」画面で確認できるURLとは異なります。
    # VPCエンドポイントのDNS名とAPI Gatewayのカスタムドメイン名（もしあれば）を組み合わせた形になります。
    # 例: https://{vpce-id}-{api-id}.execute-api.{region}[.amazonaws.com/prod/](https://.amazonaws.com/prod/)
...
4.  表示された画面で、以下の設定を行います。
    ...
    * **タスクロール**
        * 「タスクロール」: プルダウンから、手順1.1.2で作成した「**`ecsTaskRole`**」を選択します。（推奨）
        * 「タスク実行ロール」: プルダウンから、手順1.1.2で作成した「**`ecsTaskExecutionRole`**」を選択します。
...
4.  「サービスの作成」ページで以下を設定します。
    ...
    * **ネットワーキング**
        ...
        * 「パブリック IP」: 「**オフになっています**」（DISABLED）を選択します。（重要: パブリックIPを割り当てません）
